...
Given a $$w$$ bit bit vector $$x$$, we expand $$x$$ by $$k$$ bits by appending
$$k$$ `0`s to the most significant positions. That is if
...
This kind of expansion works best for `unsigned int`s and expands the bit
vector to be a $$w+k$$ bit integer, without changing the value.
