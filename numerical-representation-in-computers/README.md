...
reliably transmitted on noisy, or faulty wires, reducing the cost of error correction.
...
in hex. Then the hexadecimal number `0xfa1d37b` is `1111101000011101001101111011`
in binary and `4196218235` in decimal.
...
Are `k` bit binary numbers, and if $$\cdot$$ is a bitwise operation \(not necesarily 
those of above, then:
...
There are additional bit operations called **shift operations**. There is a **left 
shift** denoted by `<<` and a **right shift** denoted by `>>`.
...
`>>`, $$x >> w$$ overwrites the left most $$w$$ bits by the most significant 
bit of $$x$$.
...
equal the bit length is chosen. For example, `001100 << 6 = 000000`. And 
`1010 >> 4 = 0000` or `1010 >> 4 = 1111` \(depending on the context\). For that 
reason, in `C/C++` it is good to choose an integer that is less than the size 
of the given data type when using a either shift operations. In certain cases, 
it may be that the shift operation actually shifts $$k \mod{w}$$ bits, 
\(where $$w$$ is a given integer, and $$k$$ is the size of the data type\), 
hoewever it is not consistent. In some machines, only $$log_2(w)$$ bits of the 
shift ammount will actually be shifted.
