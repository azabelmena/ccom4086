...
it would help to define some things. We define the **architecture** of a 
processor to be parts of the processor design that one needs to understand and 
...
architectures**instruction set architectures**, and are used by Intel on `intel` 
processors. Mobile phones usually use `ARM` architectures. It is useful to note that 
while different processors can use the same instruction set architecture, they 
do not implement the same microarchitectures. We will focus just on the 
...
**Assembly code** is the text representation of machine code. Assembly being a 
...
executed. It consists of a **program counter** \(PC\) which stores the address of 
...
contains **condition codes**, which store the status information about arithmetic 
and logical operations, and is also used for conditional branching \(`if`, `else`
statements\). The CPU also contains the **arithmetic logic unit** \(ALU\) which 
is in charge of handling arithmetic and logical operations.
...
**heap** for allocating memory, and a **stack** to support procedures and 
...
`C/C++`, `Java`. This is done through the use of a **compiler**. A programmer can 
put code into **source** files like `p1.c` and `p2.c` and compile them with the 
...
also handles **transfer control** where it executes unconditional jumps to and from
